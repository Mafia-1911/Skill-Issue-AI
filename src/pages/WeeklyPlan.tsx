...
const WeeklyPlan = () => {
  ...
  const generateWeeklyPlan = async () => {
    ...
    try {
      ...
      const [profileRes, goalsRes, moodRes] = await Promise.all([
      ...
      // Calculate burnout data from mood check-ins
      let moodData = null;
      if (moods.length > 0) {
        const avgMood = moods.reduce((a: number, c: any) => a + c.mood_score, 0) / moods.length;
        const avgEnergy = moods.reduce((a: number, c: any) => a + c.energy_level, 0) / moods.length;
        const avgStress = moods.reduce((a: number, c: any) => a + c.stress_level, 0) / moods.length;
        const burnoutScore = Math.round(
          ((5 - avgMood) / 4 * 30 + (5 - avgEnergy) / 4 * 30 + (avgStress - 1) / 4 * 40)
        );

        const mid = Math.floor(moods.length / 2);
        const recentAvg = moods.slice(0, Math.max(1, mid)).reduce((a: number, c: any) => a + c.mood_score, 0) / Math.max(1, mid);
        const olderAvg = moods.slice(Math.max(1, mid)).reduce((a: number, c: any) => a + c.mood_score, 0) / Math.max(1, moods.length - mid);
        ...
        moodData = {
          ...
          mood: moods[0].mood_score,
          energy: moods[0].energy_level,
          stress: moods[0].stress_level,
          trend,
        };
      }
