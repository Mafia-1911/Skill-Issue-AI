...
import { Target, Clock, CheckCircle2, TrendingUp, Calendar, BookOpen, Sparkles, ArrowRight, Heart } from "lucide-react";
...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    const [profileRes, goalsRes, sessionsRes, tasksRes, checkinRes] = await Promise.all([
      ...
      supabase.from("daily_tasks").select("*").eq("user_id", user.id).eq("scheduled_date", today),
      supabase.from("mood_checkins").select("id").eq("user_id", user.id).gte("created_at", todayStart.toISOString()).limit(1),
    ]);

    setHasTodayCheckin((checkinRes.data?.length || 0) > 0);
